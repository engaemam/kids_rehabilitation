                 ...
                 <li class="list-group-item">
                        <div class="row">
                            <div class="col-xs-2 col-md-2">
                                <img src="/assets/img/{{$message->from_status==1?App\Admin::find($message->admin_id)->img : App\Sub::find($message->sub_id)->img}}" height="100" width="100" alt="user" class="rounded-circle" /></div>
                            <div class="col-xs-8 col-md-8">
                                <div>
                                    
                                    <label style="color: red;">Subject: </label>  {{$message->subject}}
                                    <div class="mic-info">
                                       <label style="color: red;"> By: </label> <a href="#">{{$message->from_status==1?'Admin\ '.App\Admin::find($message->admin_id)->name:'Sub Admin\  '.App\Sub::find($message->sub_id)->name}} </a><label style="color: red;"> on </label>{{$message->created_at}}
                                    </div>
                                </div>
			...
			<div class="modal fade" id="reply_admin" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			        <form id="ad_rep"  method="post" enctype="multipart/form-data" action="{{url('/reply/admin')}}">
			                         {{csrf_field()}}
